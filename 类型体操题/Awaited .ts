...
// 1. 判断传入的类型是否属于promise
// 2. 使用infer来判断promise内部的数据类型
// 3. 

// type MyAwaited<T extends Promise<any>> = T extends Promise<infer V> ? ( V extends Promise<infer K> ? K : V)  :never
// 这个形式可以可以处理无限层 promise类型的嵌套
type MyAwaited<T extends Promise<any>> = T extends Promise<infer V> ? V extends Promise<unknown> ? MyAwaited<V>: V :never  
